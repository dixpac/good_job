...
module GoodJob # :nodoc:
  ...
  class Scheduler
    ...
    def initialize(performer, max_threads: nil, max_cache: nil, warm_cache_on_initialize: false, cleanup_interval_seconds: nil, cleanup_interval_jobs: nil)
      ...
      @cleanup_jobs = 0 # number of jobs executed since last cleanup
      @cleanup_at = Time.current # time of last cleanup
    ...
    def cleanup?
      (@cleanup_interval_jobs && @cleanup_jobs > @cleanup_interval_jobs) ||
        (@cleanup_interval_seconds && @cleanup_at < Time.current - @cleanup_interval_seconds)
    end
